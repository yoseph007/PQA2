...
class OptionsTab(QWidget):
    ...
    def browse_logo_path(self):
        file_filter = "Images (*.png *.jpg *.jpeg *.gif *.bmp);;All Files (*.*)"
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Logo", os.path.expanduser("~"), file_filter)
        if file_path:
            self.txt_logo_path.setText(file_path)

    def toggle_white_label(self, state):
        enabled = state == Qt.Checked
        self.txt_app_name.setEnabled(not enabled)
        self.txt_company_name.setEnabled(not enabled)
        self.txt_footer_text.setEnabled(not enabled)
        self.color_primary.setEnabled(not enabled)
        self.btn_upload_logo.setEnabled(not enabled)

        # Set default values if disabled
        if not enabled:
            self.txt_app_name.setText("VMAF Test App")
            self.txt_company_name.setText("Chroma")
            self.txt_footer_text.setText("© 2025 Chroma")
            self.color_primary.setText("#4CAF50")
            self.lbl_logo_preview.setText("No logo selected")